 ...
 * 资源管理类
 * @version $Id: ResourcesManager.java 1 2016-08-08 12:54:06Z huan.song $
 * @since 1.0
 */
public class ResourcesManager {
    ...
    public int getIdentifier(String name, String type) throws NullPointerException, Resources.NotFoundException {
        if (name == null) {
            Logger.e(TAG, "getIdentifier failure, type: '" + type + "', name is null");
            throw new NullPointerException("type: '" + type + "', name is null");
        }

        name = name.trim();
        if (name.isEmpty()) {
            Logger.e(TAG, "getIdentifier failure, type: '" + type + "', name is empty");
        ...
        if (identifier <= 0) {
            Logger.e(TAG, "getIdentifier failure, type: '" + type + "', name: '" + name + "', resource not found");
