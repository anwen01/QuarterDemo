...
package com.zyk.quarterdemo.tfc.view;
...
import com.zyk.quarterdemo.tfc.util.DimensionConverter;

/**
 * SwipeUpRefreshLayout class file
 * 上拉加载更多提示布局
 * @version $Id: SwipeUpRefreshLayout.java 1 2016-10-31 14:56:06Z huan.song $
 * @since 1.0
 */
public class SwipeUpRefreshLayout extends FrameLayout {
    ...
    protected void render() {
        ...
        tvLoading.setTextSize(getTextSize());
        ...
        mTvDataEmpty = new TextView(getContext());
        mTvDataEmpty.setLayoutParams(new LinearLayout.LayoutParams(ViewGroup.LayoutParams.MATCH_PARENT, ViewGroup.LayoutParams.MATCH_PARENT));
        mTvDataEmpty.setGravity(Gravity.CENTER);
        mTvDataEmpty.setTextSize(TypedValue.COMPLEX_UNIT_SP, getTextSize());
        mTvDataEmpty.setText(getDataEmptyText());
        mTvDataEmpty.setVisibility(View.VISIBLE);
        addView(mTvDataEmpty);
    ...
    public boolean loadReady(LinearLayoutManager layoutManager) {
        ...
        if (layoutManager == null) {
            Logger.e(TAG, "isLoadMore LinearLayoutManager is null");
            return false;
        }
        ...
        Logger.d(TAG, "isLoadMore totalCount: " + totalCount + ", lastPos: " + lastPos + ", showCount: " + showCount);
     ...
     * @return 提示语
     */
    public String getDataEmptyText() {
        return mDataEmptyText;
    }
    ...
    public boolean isNoData() {
        return getStatus() == STATUS_DATA_EMPTY;
    }
    ...
    public boolean setStatus(int status) {
        if (status != STATUS_LOADING && status != STATUS_LOAD_MORE && status != STATUS_DATA_EMPTY) {
            return false;
        }
    ...
    public static final class Builder {
         ...
         *
         * @return a SwipeUpRefreshLayout
         */
        public SwipeUpRefreshLayout create() {
            SwipeUpRefreshLayout result = new SwipeUpRefreshLayout(mContext);

            DimensionConverter converter = DimensionConverter.getInstance(mContext);

            result.mLoadingText = mLoadingText;
            result.mLoadMoreText = mLoadMoreText;
            result.mDataEmptyText = mNoDataText;
            result.mLayoutHeight = (int) converter.dp2px(mLayoutHeight);
            result.mPrgBarHeight = (int) converter.dp2px(mPrgBarHeight);
